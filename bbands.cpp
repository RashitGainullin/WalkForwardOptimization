...
Rcpp::List bbands(
  ...
  bool   is_inverse = parameters["is_inverse"];
  ...
  bt.onTick = [&]( Tick tick ) {
    ...
    if( state == ProcessingState::FLAT) {
      // if price below lower band then buy or short if is_inverse
      if( tick.price < bbands.GetValue().lower & is_inverse == false ) {
      ...
      if( tick.price < bbands.GetValue().lower & is_inverse == true ) {
      ...
      // if price above upper band then sell or long if is_inverse
      if( tick.price > bbands.GetValue().upper & is_inverse == false ) {
      ...
      if( tick.price > bbands.GetValue().upper & is_inverse == true ) {
    ...
    // if strategy is long, not inverse and price goes above sma then close long
    if( state == ProcessingState::LONG and tick.price > bbands.GetValue().sma & is_inverse == false ) {
    ...
    // if strategy is below, not inverse and price goes below sma then close long
    if( state == ProcessingState::SHORT and tick.price < bbands.GetValue().sma & is_inverse == false ) {
    ...
    // if strategy is below, inverse and price goes above sma then close long
    if( state == ProcessingState::SHORT and tick.price > bbands.GetValue().sma & is_inverse == true ) {
    ...
    // if strategy is long, inverse and price goes below sma then close long
    if( state == ProcessingState::LONG and tick.price < bbands.GetValue().sma & is_inverse == true ) {
